...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.pub = self.create_publisher(Sensor, 'status/scanner_on', 10)
    ...
    def callback(self,data):
        ...
        c.open()
        print(data.position)
        if data.position > 179:
            c.write(('Ros_test',1))
            c.write(('send_to_micrologix[0]',32) )  
            c.write(('send_to_micrologix[1]',1)  ) 
            c.write(('send_to_micrologix[2]',255) )
            time.sleep(50) 
            c.write(('Ros_test',0))

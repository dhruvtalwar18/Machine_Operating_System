...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(MAMstatus ,'status/xaxis',self.callback,10)
        self.subscription
        self.pub = self.create_publisher(MAMstatus, 'status/zaxis', 10)
    ...
    def callback(self,data):
        ...
        #c.open() 
        print('hello')
        if data.position < -184:
            c.open()
            c.write(('Z_mov_type', 0), ('Z_Ros_Position', 179.5), ('Z_Ros_Speed', 5))
            print("1")
